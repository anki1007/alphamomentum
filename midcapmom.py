"""
Midcap 50 Strategy - Bloomberg-Style Performance Dashboard
...
# Page configuration
st.set_page_config(
    page_title="Midcap 50 - Performance Dashboard",
    page_icon="📈",
    layout="wide",
...
class MidcapShopBacktester:
    ...
    def __init__(
        ...
        self.min_slope_angle = float(min_slope_angle)
        self.max_slope_angle = float(max_slope_angle)
    ...
    def run_backtest(self, progress_callback=None) -> pd.DataFrame:
        # Precompute DMA and trend indicators
        for sym, df in self.data.items():
            ...
            if self.use_trend_filter:
                df["63DMA"] = self.compute_moving_average(df, 63, "Close")
                df["126DMA"] = self.compute_moving_average(df, 126, "Close")
                df["252DMA"] = self.compute_moving_average(df, 252, "Close")
                
                # Calculate 252DMA slope angle
                # slope = (current - N days ago) / N days ago as percentage
                df["252DMA_prev"] = df["252DMA"].shift(self.slope_lookback)
                df["252DMA_slope_pct"] = np.where(
                    df["252DMA_prev"].notna() & (df["252DMA_prev"] > 0),
                    (df["252DMA"] - df["252DMA_prev"]) / df["252DMA_prev"] * 100,
                    0.0
                )
                
                # Convert percentage slope to angle (degrees)
                # Scale factor: 1% change over lookback period ≈ certain angle
                # Typical scaling: multiply by (252/lookback) to annualize, then arctan
    ...
    def _get_top5_below_20dma(self, dt: pd.Timestamp) -> List[str]:
        ...
        for sym, df in self.data.items():
            if dt not in df.index:
                continue
            row = df.loc[dt]
            pct = row["pct_below_20dma"]
            
            # Check trend filter if enabled
            trend_ok = row.get("trend_valid", True)
            if pd.isna(trend_ok):
                trend_ok = False
            
            if not pd.isna(pct) and pct > 0 and trend_ok:
...
def fetch_nifty_Midcap50_stocks():
    """Fetch current Midcap 50 Midcap 50 constituents"""
    ...
    # Default list if fetch fails - Nifty Midcap 50 constituents (Jan 2025)
    return [
        'APLAPOLLO', 'AUBANK', 'ASHOKLEY', 'AUROPHARMA', 'BSE',
        'BHARATFORG', 'BHEL', 'COFORGE', 'COLPAL', 'CUMMINSIND',
        'DABUR', 'DIXON', 'FEDERALBNK', 'FORTIS', 'GMRAIRPORT',
        'GODREJPROP', 'HDFCAMC', 'HEROMOTOCO', 'HINDPETRO', 'IDFCFIRSTB',
        'IRCTC', 'INDUSTOWER', 'INDUSINDBK', 'JUBLFOOD', 'LUPIN',
        'MANKIND', 'MARICO', 'MFSL', 'MPHASIS', 'MUTHOOTFIN',
        'NHPC', 'NMDC', 'OBEROIRLTY', 'OIL', 'PAYTM',
        'OFSS', 'POLICYBZR', 'PIIND', 'PAGEIND', 'PERSISTENT',
        'PHOENIXLTD', 'POLYCAB', 'PRESTIGE', 'SBICARD', 'SRF',
        'SUPREMEIND', 'SUZLON', 'TIINDIA', 'UPL', 'YESBANK'
    ]
...
def run_daily_screener(symbols: List[str], progress_callback=None) -> pd.DataFrame:
    ...
    for i, symbol in enumerate(symbols):
        ...
        try:
            ...
            df['63DMA'] = df['Close'].rolling(window=63).mean()
            df['126DMA'] = df['Close'].rolling(window=126).mean()
            ...
            annualized_slope = df['252DMA_slope_pct'] * (252 / slope_lookback)
            df['252DMA_angle'] = np.degrees(np.arctan(annualized_slope / 100))
            ...
            latest_dma63 = float(latest['63DMA']) if not pd.isna(latest['63DMA']) else None
            ...
            results.append({
                ...
                '63 DMA': round(latest_dma63, 2) if latest_dma63 else '-',
...
def main():
    ...
    <div style="display: flex; align-items: center; justify-content: space-between; padding: 10px 0;">
        ...
        <div style="text-align: right;">
            <div style="font-size: 14px; color: #00d4ff;">Midcap 50 Strategy</div>
        </div>
    </div>
    ...
    with tab1:
        ...
        st.markdown("*Find stocks from Midcap 50 Midcap 50 that are below their 20-Day Moving Average*")
        ...
        if run_screener or 'screener_df' in st.session_state:
            ...
            if not screener_df.empty:
                ...
                def highlight_trend(val):
                    if '✅' in str(val):
                        ...
                        return 'background-color: rgba(255, 170, 0, 0.3); color: #ffaa00'
                    elif '❌' in str(val):
                ...
                with col1:
                    ...
                    st.download_button(
                        ...
                        data=csv_screener,
                        file_name=f"Midcap50_screener_{date.today()}.csv",
                        mime="text/csv"
                    )
                with col2:
            ...
            <div style="text-align: center; padding: 50px;">
                ...
                <p style="color: #888;">
                    The screener will analyze all 50 stocks in the Midcap 50 Midcap 50 index and identify
                    those trading below their 20-Day Moving Average - potential buying opportunities
                    according to the Midcap Shop strategy.
                </p>
    ...
    with tab2:
        ...
        with st.sidebar:
            ...
            if use_trend_filter:
                ...
                min_slope = st.slider("Min Slope Angle (°)", 0.0, 15.0, 5.0, 1.0, key="bt_min_slope",
                                     help="Minimum uptrend angle")
                max_slope = st.slider("Max Slope Angle (°)", 15.0, 45.0, 30.0, 1.0, key="bt_max_slope",
                                     help="Maximum angle (avoid parabolic)")
            else:
                slope_lookback = 20
                min_slope = 5.0
                max_slope = 30.0
        ...
        if run_backtest or 'metrics' in st.session_state:
            if run_backtest:
                ...
                with st.spinner("Fetching stock list..."):
                    instruments = fetch_nifty_Midcap50_stocks()
                    st.info(f"Loaded {len(instruments)} stocks from Midcap 50")
            ...
            st.selectbox("Choose Strategy", ["Midcap 50"], key="strategy_select")
            ...
            with col1:
                if not trades_df.empty:
                    ...
                    st.download_button(
                        ...
                        data=csv,
                        file_name=f"Midcap50_tradebook_{date.today()}.csv",
                        mime="text/csv"
                    )
            ...
            with col2:
                if metrics:
                    ...
                    st.download_button(
                        ...
                        data=excel_data,
                        file_name=f"Midcap50_report_{date.today()}.xlsx",
            ...
            with col3:
                ...
                if metrics:
                    ...
                    st.download_button(
                        ...
                        file_name=f"Midcap50_metrics_{date.today()}.json",
            ...
            if run_optimization:
                ...
                try:
                    ...
                    temp_backtester = MidcapShopBacktester(
                        ...
                        dma_window=opt_dma,
                        max_avg=opt_max_pos,
                        use_trend_filter=opt_use_trend,
                        slope_lookback=opt_slope_lookback,
                        min_slope_angle=opt_min_slope,
                        max_slope_angle=opt_max_slope
                    )
