"""
Alpha Momentum Pro - COMPLETE FIXED CODE
Production-Ready Version with All 3 Critical Fixes Applied

FIXES INCLUDED:
1. ✅ Added missing: from plotly.subplots import make_subplots
2. ✅ New function: dataframe_to_markdown() - eliminates tabulate dependency
3. ✅ Modern CSV export using io.StringIO() - future-proof
"""
...
# ═══════════════════════════════════════════════════════════════════════════
# ✅ KEY FIX #1: CUSTOM MARKDOWN FUNCTION (Lines 56-80)
# Replaces broken df.to_markdown() - no external dependencies needed
# ═══════════════════════════════════════════════════════════════════════════
...
with st.sidebar:
    ...
    universe_choice = st.selectbox("Select Universe", options=list(CSV_FILES.keys()), index=0)
    benchmark_choice = st.selectbox("Select Benchmark", options=list(BENCHMARKS.keys()), index=0)
    period_choice = st.selectbox("Historical Period", options=["1y", "2y", "3y", "5y"], index=1)
    portfolio_size = st.slider("Portfolio Size (Top N stocks)", min_value=5, max_value=100, value=20, step=5)
...
if run_screener:
    ...
    with st.spinner("Fetching price data..."):
        raw_data = fetch_prices(universe_df["Symbol"].tolist(), BENCHMARKS[benchmark_choice], period_choice)
        if raw_data.empty:
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    # ✅ KEY FIX #2: CUSTOM MARKDOWN FUNCTION (replaces broken df.to_markdown())
    # ═══════════════════════════════════════════════════════════════════════════
    markdown_table = dataframe_to_markdown(display_df.drop(columns=["Symbol", "Chart"], errors="ignore"))
    st.markdown(markdown_table)
    
    # ═══════════════════════════════════════════════════════════════════════════
    # ✅ KEY FIX #3: MODERN CSV EXPORT using io.StringIO()
    # ═══════════════════════════════════════════════════════════════════════════
    ...
    with col1:
        fig_ret = px.bar(display_df.head(15), x="Name", y="Return_6M", title="Top 15 - 6M Returns",
                        color="Return_6M", color_continuous_scale="RdYlGn")
        fig_ret.update_layout(height=400, showlegend=False)
        st.plotly_chart(fig_ret, use_container_width=True)
    with col2:
        fig_quad = px.scatter(display_df, x="RS-Ratio", y="RS-Momentum", color="Performance",
                             size="Return_6M", hover_name="Name", title="RS-Ratio vs RS-Momentum Quadrant")
        fig_quad.add_hline(y=100, line_dash="dash", line_color="gray")
        fig_quad.add_vline(x=100, line_dash="dash", line_color="gray")
